    const { Client, IntentsBitField, ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, EmbedBuilder, DefaultWebSocketManagerOptions } = require('discord.js');
    ...
    async function startGame(gameChannel) {
        ...
        if (firstCard.type == "wild" || firstCard.type == "+4") {
        ...
        switch(firstCard.type) {
            ...
            case "+2": {
                for (let i = 0; i < 2; i++) {
                    ...
                    game.hands[game.turnOrder[game.currentPlayerIndex]].push(drawnCard);
                    game.currentPlayerIndex = (game.currentPlayerIndex + game.direction) % game.turnOrder.length;
                    advanceTurn(gameChannel);
                }
                break;
